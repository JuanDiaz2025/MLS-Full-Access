...
2. **Scan** — `CITIES="Alameda:Oakland;Contra Costa:Richmond;…" node scripts/mls-multi-scan.js` → `multi-scan.json`. Matrix: Active · SFR · ≤ cap ($2.0M Peninsula, else $1.5M) · ≤45 DOM.
3. **Filter** — `MODE=filter node scripts/mls-flipscout.js` → `.mls-artifacts/candidates.json` (below-market $/sf + old; drops new builds, large-home $/sf traps, DOM>45).
...
5. **Profile-verify** — `MLS_LIST="…" node scripts/mls-profile.js` → `.mls-artifacts/profiles-out.json` (zip + remarks + tenant/fire/multi-unit/probate flags). Drop: renovated · DOM>45 · tenant-occupied · multi-unit/duplex/2nd-unit · vacant lot · fire-damaged.
